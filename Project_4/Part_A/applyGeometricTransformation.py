...
def applyGeometricTransformation(startX, startY, newXs, newYs, bbox):
    ...
    for i in range(0, num_boxes):
        ...
        trans = cv2.estimateRigidTransform(np.array(zip(keepSX, keepSY)).astype(int), np.array(zip(keepEX, keepEY)).astype(int), False)

        nbox = [np.round(np.dot(trans,[pt[0], pt[1], 1])) for pt in bboxc[i]]
